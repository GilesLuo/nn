...
if not os.path.isdir('./sequential_n'):
    os.mkdir('./sequential_n')
...
if __name__ == '__main__':
    ...
    for i in range(len(hidden_list)):
        ...
        print(loss)
        imageio.mimwrite('./sequential_n/lr=0.05_hidden=' + str(hidden_list[i]) + '_epoch=' + str(t) + '.gif', frames, duration=0.02)
        # loss = np.array(loss)
        plt.title('lr=0.05_hidden=' + str(hidden_list[i]) + '_epoch=' + str(t))
        plt.xlabel('num_epoch')
        plt.ylabel('loss value')
        plt.plot(range(len(loss)), loss)
        plt.savefig('./sequential_n/lr=0.05_hidden=' + str(hidden_list[i]) + '_epoch=' + str(t) + '.png')
        plt.close()
